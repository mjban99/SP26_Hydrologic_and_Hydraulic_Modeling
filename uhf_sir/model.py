...
class Model():
    ...
    def _initial_channel_state(self):
        ...
        return np.concatenate([h_init, Q_init, AC_init])

    def _channel_rhs(self, t, y, qL_interp, C_lat_interp, n):
        """
        RHS of coupled Saint-Venant + ADE system, Eq. A.9-A.12.

        State vector y = [h_1..h_N, Q_1..Q_N, (AC)_1..(AC)_N].
        """
        # Import instance variables
        N = self.N
        B = self.B
        ...
        C = np.maximum(C, 0.0)
        # Friction slope, Eq. A.11
        Sf = n * n * Q * np.abs(Q) / (A * A * R ** (4.0 / 3.0) + 1e-12)
        ...
        # Momentum, diffusion-wave form (inertia neglected)
        h_ext = np.concatenate(([h[0]], h, [h[-1]]))
        dhdx = (h_ext[:-2] - h_ext[2:]) / (2.0 * dx)
        dQ = g * A * (dhdx + S_0 - Sf)
        # ADE advection, Eq. A.12 (upwind based on Q_face sign)
    ...
    def _solve_channel(self, force_alpha_one=False, n_mixing_iters=2,
        ...
        """
        Solve coupled Saint-Venant + ADE channel routing.

        A fixed-point iteration is used over the Q-dependent mixing:
        the channel is solved with a current guess of Q, the mixing
        weight alpha(Q) is updated, and the channel is re-solved.
        Two iterations are sufficient for convergence at hourly
        output resolution.
        ...
        # Initial channel state under Manning steady baseflow
        ...
        for _ in range(n_mixing_iters):
            ...
            if not sol.success:
                break
            # Update Q guess from this solve (outlet cell, last column)
            Q_outlet = sol.y[2 * self.N - 1]
        ...
        if sol is None or not sol.success:
            ...
            return False
        h_outlet = sol.y[self.N - 1]
        Q_outlet = sol.y[2 * self.N - 1]
        AC_outlet = sol.y[3 * self.N - 1]
