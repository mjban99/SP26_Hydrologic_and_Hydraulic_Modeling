...
def plot_mass_balance(storm_data, cl_rain_t, cl_rain, cl_stream_t, cl_stream,
    ...
    # Rainfall-weighted mean rainfall Cl on the hourly grid
    # (linear interpolation of 7-hourly samples → hourly weighting by rain depth)
    rain_flux_h = storm_data['rain_flux'][in_win]      # mm/h on hourly grid
    rain_t_h = t_h[in_win]                             # hourly times within window
    cl_rain_interp = np.interp(rain_t_h,
                                cl_rain_t[rain_in],
                                cl_rain[rain_in])      # mg/L on hourly grid
    total_rain = rain_flux_h.sum()                     # mm
    if total_rain > 0:
        wmean = np.sum(rain_flux_h * cl_rain_interp) / total_rain
    else:
        wmean = np.nan
    # Figure
    if fig is None:
